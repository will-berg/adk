 ...
 * omvandlade till sm bokstver, ett ord per rad fljt av ordets
 * teckenposition i texten. Ett ord definieras som en eller flera
 * p varandra fljande bokstver i det svenska alfabetet.
 * Copyright Viggo Kann, viggo@nada.kth.se, 1999
 *
 * Ltt omhackad av Jakob Nordstrm 2003 fr att klara tecken med accenter
 * (t.ex. , .  osv.).
 *
 */
...
Initialize(void)
{
    ...
    /*
     * Nedan fljer speciallsning fr att klara accenterade tecken
     *
     */
    ...
    for (ch = 242; ch <= 245; ++ch) /* o med accent (frutom ) */
    {
        u2l[ch +  - 'a' + 'A'] = u2l[ch] = 'o';
    }
